	...
	constructor() {
		...
		this.version(2)
			.stores({
				...
				members: "++id, &memberNumber, &cloudId, name, cooperativeId, syncedAt",
		...
		this.version(SCHEMA_VER)
			.stores({
				...
				tasks: "++id, status, dueDate, createdAt, cloudId, syncedAt",
				customers:
					"++id, &cloudId, name, phone, email, category, status, syncedAt",
				suppliers:
					"++id, &cloudId, name, phone, email, category, status, syncedAt",
				procurements: "++id, &cloudId, date, supplierId, status, syncedAt",
			})
			.upgrade((tx) => {
				return tx
					.table("syncQueue")
					.toCollection()
					.modify((item) => {
						if (item.attemptCount === undefined) item.attemptCount = 0;
		...
		this.procurements = this.table("procurements");
	}
}
