	...
	return (
		<div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
			{cards.map((card) => (
				<div
					key={card.label}
					className="rounded-2xl bg-white border border-[#D8E4EA] shadow-soft p-5"
				>
					<div className="flex items-center justify-between mb-3">
						<span className={`w-10 h-10 ${card.bg} rounded-xl grid place-items-center`}>
							{card.icon}
						</span>
					</div>
					<p className={`font-display font-extrabold text-[28px] tracking-tight ${card.valueColor || "text-[#0F172A]"}`}>
						{card.value}
					</p>
					<p className="text-[13px] text-[#475569] mt-0.5">{card.label}</p>
					{card.change && (
						<p className="text-[11px] text-[#22C55E] font-semibold mt-1">{card.change}</p>
					)}
				</div>
			))}
		</div>
