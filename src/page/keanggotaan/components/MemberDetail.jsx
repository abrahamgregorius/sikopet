				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full mt-1 ${
							member.status === "active" ? "text-[#22C55E] bg-[#22C55E]/10" :
							member.status === "pending" ? "text-[#F59E0B] bg-[#F59E0B]/10" :
							...
							{member.status === "active" ? "Aktif" : member.status === "pending" ? "Menunggu" : "Tidak Aktif"}
						</span>
					</div>
				</div>
				...
				<div className="space-y-3">
					...
					<div className="space-y-2.5">
						{[
							...
							{ label: "Tempat & Tgl Lahir", value: `${member.address.split(",")[0]}, ${member.bornDate}` },
							{ label: "Alamat", value: member.address },
							{ label: "Telepon", value: member.phone },
							{ label: "Tanggal Bergabung", value: member.joinDate },
						].map((item) => (
							<div key={item.label} className="flex justify-between text-[13.5px] py-1.5 border-b border-[#E8EEF2] last:border-0">
								<span className="text-[#94A3B8]">{item.label}</span>
								<span className="font-medium text-[#0F172A] text-right">{item.value}</span>
							</div>
						))}
					</div>
				</div>
