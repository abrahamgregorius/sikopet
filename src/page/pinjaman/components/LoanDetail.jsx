		...
		<div className="rounded-lg bg-white border border-[#E5E7EB] overflow-hidden">
			...
			<div className="p-5 space-y-5">
				...
				<div className="rounded-lg bg-[#F1F5F9] p-4">
					...
					<div className="h-2.5 rounded-lg bg-[#D8E4EA] overflow-hidden">
						<div
							className="h-full rounded-lg bg-gradient-to-r from-[#398eb3] to-[#4CC9B0] transition-all"
				...
				<div className="grid grid-cols-2 gap-3">
					{[
						...
						{
							label: "Cicilan/bulan",
							value: `Rp ${Math.round(monthlyPayment).toLocaleString("id-ID")}`,
						},
						{ label: "Sisa", value: `Rp ${remaining.toLocaleString("id-ID")}` },
					].map((item) => (
						<div key={item.label} className="rounded-lg bg-[#F1F5F9] p-3.5">
							<p className="text-[11px] text-[#94A3B8]">{item.label}</p>
							<p className="text-[14px] font-bold text-[#0F172A] mt-0.5">
								{item.value}
							</p>
						</div>
					))}
				...
				<div className="space-y-2.5">
					...
					{loan.status === "active" && loan.paid > 0 ? (
						<div className="rounded-lg bg-[#DCFCE7] p-3.5 text-center">
							...
							<p className="text-[12px] text-[#22C55E]/70 mt-0.5">
								Berikutnya: Rp{" "}
								{Math.round(monthlyPayment).toLocaleString("id-ID")} per bulan
							</p>
						</div>
					) : loan.status === "pending" ? (
						<div className="rounded-lg bg-[#FEF3C7] p-3.5 text-center">
