				...
				<div className={`nested-panel ${isFilterOpen ? "nested-open" : ""}`}>
					<div className="flex flex-wrap gap-2 pt-4">
						{["Berhasil", "Tertunda", "Gagal"].map((f) => (
							<label
								key={f}
								className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#F1F5F9] text-[12.5px] font-medium text-[#475569] cursor-pointer"
							>
								<input
									type="checkbox"
					...
					<table className="w-full text-left">
						<thead>
							<tr className="border-y border-[#E5E7EB] bg-[#F1F5F9]/60">
								...
								<th
									scope="col"
									className="px-6 py-3 text-[11.5px] font-bold text-[#94A3B8] uppercase tracking-wide text-right"
								>
									Aksi
								</th>
							</tr>
						...
						<tbody className="divide-y divide-[#E8EEF2]">
							{filtered.map((tx) => (
								<tr key={tx.id} className="hover:bg-[#F1F5F9]/50">
									<td className="px-6 py-3.5 text-[13.5px] font-semibold text-[#0F172A]">
										{tx.name}
									</td>
									<td className="px-6 py-3.5 text-[13.5px] text-[#475569]">
										{tx.type}
									</td>
									<td className="px-6 py-3.5 text-[13.5px] font-semibold text-[#0F172A]">
										{tx.amountFormatted}
