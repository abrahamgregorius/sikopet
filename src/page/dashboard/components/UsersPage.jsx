...
export default function UsersPage() {
    ...
    async function loadUsers() {
        ...
        try {
            ...
            setUsers(allUsers.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
        } catch (err) {
    ...
    function resetForm() {
        setForm({ name: "", email: "", password: "", role: "operator", cooperativeId: 1 });
        setFormError("");
        setEditingUser(null);
    }
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <button
                    ...
                    className="focus-ring inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#398eb3] text-white font-semibold text-[14px] hover:bg-[#2F7A9A] transition-colors shadow-glow"
                >
            ...
            <div className="rounded-2xl bg-white border border-[#D8E4EA] overflow-hidden">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-48 text-center px-6">
                        <div className="w-14 h-14 rounded-2xl bg-[#F1F5F9] grid place-items-center mb-4">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="1.8">
                                <circle cx="9" cy="8" r="3" /><path d="M2 20a7 7 0 0114 0M16 6a3 3 0 010 6M22 20a7 7 0 00-6-6.9" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        </div>
                        <p className="text-[14px] text-[#94A3B8]">Tidak ada karyawan ditemukan</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-[#E8EEF2]">
                                    <th className="text-left px-5 py-3.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Nama</th>
                                    <th className="text-left px-5 py-3.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Email</th>
                                    <th className="text-left px-5 py-3.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Role</th>
                                    <th className="text-left px-5 py-3.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Bergabung</th>
                                    <th className="text-right px-5 py-3.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredUsers.map((user) => (
                                    <tr key={user.id} className="border-b border-[#E8EEF2] last:border-0 hover:bg-[#F7FAFB] transition-colors">
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#398eb3] to-[#2F7698] grid place-items-center text-white text-[13px] font-bold">
                                                    {user.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
                                                </div>
                                                <span className="font-medium text-[14px] text-[#0F172A]">{user.name}</span>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4 text-[14px] text-[#6B7280]">{user.email}</td>
                                        <td className="px-5 py-4">
                                            <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold ${ROLE_COLORS[user.role] || "text-[#94A3B8] bg-[#F1F5F9]"}`}>
            ...
            {isModalOpen && (
                <>
                    <div onClick={() => setIsModalOpen(false)} className="fixed inset-0 bg-[#0F172A]/40 z-[80]" />
                    <div className="fixed inset-0 z-[90] grid place-items-center px-4">
                        <div className="w-full max-w-[440px] bg-white rounded-3xl shadow-lift p-6 sm:p-7">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    <label className="block text-[12.5px] font-semibold text-[#475569] mb-1.5">Nama Lengkap</label>
                                    <input
                                        type="text"
                                        value={form.name}
                                ...
                                <div>
                                    <label className="block text-[12.5px] font-semibold text-[#475569] mb-1.5">Role / Jabatan</label>
                                    <select
                                        value={form.role}
                                ...
                                {formError && (
                                    <div className="rounded-xl bg-[#FEE2E2] border border-[#FECACA] px-4 py-3">
                                        <p className="text-[13px] text-[#EF4444] font-medium">{formError}</p>
                                    </div>
                                )}
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button
                                        type="button"
                                        onClick={() => setIsModalOpen(false)}
                                        className="focus-ring flex-1 h-[46px] rounded-full border border-[#D8E4EA] font-semibold text-[14px] text-[#475569] hover:bg-[#F1F5F9] transition-colors"
                                    >
                                        Batal
                                    </button>
                                    <button
                                        type="submit"
                                        className="focus-ring flex-1 h-[46px] rounded-full bg-[#398eb3] text-white font-semibold text-[14px] hover:bg-[#2F7A9A] transition-colors shadow-glow"
                                    >
