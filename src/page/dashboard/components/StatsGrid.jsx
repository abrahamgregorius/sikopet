...
function StatCounter({ end, decimals = 0, suffix = "", prefix = "", format = "full", isLoading }) {
...
const STAT_CONFIGS = [
  { key: "totalRevenue", prefix: "Rp ", format: "short", label: "Total Pendapatan", icon: "revenue" },
...
export default function StatsGrid({ refreshTrigger, onRefresh }) {
  ...
  const statsList = STAT_CONFIGS.map((cfg) => ({
    ...cfg,
    end: cfg.key === "totalRevenue" || cfg.key === "totalMembers" ? (stats[cfg.key] || 0) : (stats[cfg.key] || 0),
  }));
    ...
    <section aria-label="Statistik ringkasan" className="reveal in">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={onRefresh}
          data-tip="Muat ulang"
          className="focus-ring p-2 rounded-full hover:bg-[#F1F5F9] text-[#94A3B8] hover:text-[#0F172A] transition-colors"
          ...
          >
            <div className="flex items-start justify-between">
              <span className="w-10 h-10 rounded-xl bg-[#EAF6FB] grid place-items-center">
                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#398eb3" strokeWidth="1.8">
                  {icons[stat.icon]}
                </svg>
              </span>
            </div>
