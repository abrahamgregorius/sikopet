				...
				Object.entries(grouped).map(([category, mods]) => (
					<div key={category} className="space-y-3">
						<div className="flex items-center gap-2">
							...
							<span
								className={`text-[11px] font-semibold px-2 py-0.5 rounded-lg ${CATEGORY_COLORS[category]}`}
							>
								{mods.length}
							</span>
						</div>
									...
									>
										<div className="flex items-start justify-between mb-3">
											...
											>
												<svg
													...
													viewBox="0 0 24 24"
													fill="none"
													stroke={mod.comingSoon ? "#F59E0B" : "#398eb3"}
													strokeWidth="1.8"
													strokeLinecap="round"
													strokeLinejoin="round"
													className="shrink-0"
											...
											<div className="flex items-center gap-2">
												{mod.comingSoon && (
													<span className="text-[10px] font-bold text-[#F59E0B] bg-[#F59E0B]/10 px-2 py-0.5 rounded-lg uppercase">
														Coming Soon
													</span>
												)}
												<span
													className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2 py-0.5 rounded-lg ${colors.bg} ${colors.text}`}
												>
													<span
														className={`w-1.5 h-1.5 rounded-lg ${colors.dot}`}
													/>
													{status === "enabled"
														? "Aktif"
														: status === "disabled"
										...
										{!mod.comingSoon && mod.key !== "moduleManager" && (
											<button
												...
												className={`focus-ring w-full h-[36px] rounded-lg font-semibold text-[13px] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
													mod.enabled
