					...
					return (
						<div key={category}>
							...
							<ul className="space-y-1">
								{items.map((item) => (
									<li key={item.key} className={item.nested ? `nested-group ${openNested === item.key ? "nested-open" : ""}` : ""}>
										{item.nested ? (
											<button
												onClick={() => setOpenNested(openNested === item.key ? null : item.key)}
												className={`nested-toggle w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-[14px] text-[#475569] hover:bg-[#F1F5F9] transition-colors focus-ring ${collapsed ? "justify-center" : ""}`}
											...
											>
												...
												{!collapsed && (
													<>
														...
														<svg className="chev-nested w-3.5 h-3.5 shrink-0 transition-transform duration-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
				...
				{!collapsed && (
					<div className="flex items-center gap-2.5 rounded-xl bg-[#EAF6FB] px-3.5 py-3 mb-3">
						<span className="relative w-2.5 h-2.5 shrink-0">
							<span className={`absolute inset-0 rounded-full ${online ? "bg-[#22C55E]" : "bg-[#EAB308]"}`}></span>
							{online && <span className="absolute inset-0 rounded-full bg-[#22C55E] animate-ping opacity-60"></span>}
						</span>
						<div className="min-w-0">
							<p className="text-[12px] font-semibold text-[#2F7698]">{online ? "Sinkron Aktif" : "Offline"}</p>
							<p className="text-[10.5px] text-[#2F7698]/70">{online ? `Terakhir ${formatTimeAgo(lastSync)}` : "Tidak tersinkron"}</p>
						</div>
					</div>
				)}
				<button
					...
					className="focus-ring w-full flex items-center justify-center gap-2 py-2 rounded-xl text-[#94A3B8] hover:bg-[#F1F5F9] hover:text-[#0F172A] transition-colors"
