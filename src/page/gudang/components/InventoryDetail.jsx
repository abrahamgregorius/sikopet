		...
		<div className="rounded-lg bg-white border border-[#E5E7EB] overflow-hidden">
			...
			<div className="p-5 space-y-5">
				<div className="flex items-center gap-4">
					<div className="w-14 h-14 rounded-lg bg-[#EAF6FB] grid place-items-center">
						<svg
							width="24"
							height="24"
							viewBox="0 0 24 24"
				...
				<div className="rounded-lg bg-[#F1F5F9] p-4">
					...
					<p
						className={`font-display font-extrabold text-[28px] tracking-tight ${isLow ? "text-[#EF4444]" : "text-[#0F172A]"}`}
					>
						{item.stock}{" "}
						<span className="text-[16px] text-[#94A3B8]">{item.unit}</span>
					</p>
					<div className="mt-3 h-2 rounded-lg bg-[#D8E4EA] overflow-hidden">
						<div
							className={`h-full rounded-lg transition-all ${isLow ? "bg-[#EF4444]" : "bg-gradient-to-r from-[#398eb3] to-[#4CC9B0]"}`}
							style={{
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<div className="rounded-lg bg-[#F1F5F9] p-3.5 col-span-2">
				...
				<div className="flex gap-3">
					<input
						...
						className="focus-ring flex-1 h-10 px-3 rounded-lg border border-[#E5E7EB] bg-white text-[14px] text-[#0F172A] placeholder-[#9CA3AF] focus:border-[#398EB3] focus:outline-none transition-colors"
					/>
					<button
						onClick={handleRestock}
						className="focus-ring h-10 px-4 rounded-lg bg-[#398EB3] text-white font-semibold text-[14px] hover:bg-[#2F7A9A] transition-colors"
					>
						Restok
					</button>
				</div>
