	...
	return (
		<div className="rounded-2xl bg-white border border-[#D8E4EA] shadow-soft overflow-hidden">
			<div className="p-5 border-b border-[#E8EEF2] flex items-center justify-between">
				<h3 className="font-display font-bold text-[16px] text-[#0F172A]">Detail Pemasok</h3>
			...
			<div className="p-5 space-y-5">
				<div className="flex items-center gap-4">
					<span className="w-14 h-14 rounded-full bg-[#FEF3C7] grid place-items-center text-[#F59E0B] font-bold text-[20px]">{supplier.name.charAt(0)}</span>
					<div>
						<p className="font-display font-bold text-[18px] text-[#0F172A]">{supplier.name}</p>
						<span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full mt-1 ${supplier.status === "active" ? "text-[#22C55E] bg-[#DCFCE7]" : supplier.status === "pending" ? "text-[#F59E0B] bg-[#FEF3C7]" : "text-[#94A3B8] bg-[#F1F5F9]"}`}>
							{supplier.status === "active" ? "Aktif" : supplier.status === "pending" ? "Menunggu" : "Tidak Aktif"}
						</span>
					</div>
				</div>
				...
				<div className="rounded-xl bg-[#F1F5F9] p-4 space-y-3">
					<div className="flex justify-between text-[13px]">
						...
						<span className="font-medium text-[#0F172A]">{supplier.category}</span>
					</div>
					<div className="flex justify-between text-[13px]">
						<span className="text-[#94A3B8]">Kontak</span>
						<span className="font-medium text-[#0F172A]">{supplier.contact}</span>
					</div>
					<div className="flex justify-between text-[13px]">
				...
				<div className="grid grid-cols-2 gap-3">
					<div className="rounded-xl bg-[#F1F5F9] p-3.5">
						...
						<p className="text-[15px] font-bold text-[#0F172A] mt-0.5">Rp {(supplier.totalTransaction / 1000000).toFixed(1)}jt</p>
					</div>
					<div className="rounded-xl bg-[#F1F5F9] p-3.5">
						<p className="text-[11px] text-[#94A3B8]">Rating</p>
						<p className="text-[15px] font-bold text-[#0F172A] mt-0.5">{supplier.rating > 0 ? supplier.rating.toFixed(1) : "-"}</p>
					</div>
				</div>

				<div className="rounded-xl bg-[#F1F5F9] p-3.5">
					<p className="text-[11px] text-[#94A3B8]">Alamat</p>
					<p className="text-[13px] font-medium text-[#0F172A] mt-0.5">{supplier.address}</p>
				</div>
			</div>
