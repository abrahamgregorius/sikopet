...
export default function SupplierList({ suppliers, search, categoryFilter, categories, onSearch, onCategoryFilter, onSelect, selectedId }) {
	return (
								...
								>
									<td className="px-5 py-3.5">
										<div className="flex items-center gap-3">
											<span className="w-9 h-9 rounded-full bg-[#FEF3C7] grid place-items-center text-[#F59E0B] font-bold text-[13px]">{s.name.charAt(0)}</span>
											<div>
												<p className="text-[14px] font-semibold text-[#0F172A]">{s.name}</p>
												<p className="text-[11px] text-[#94A3B8]">{s.contact}</p>
											</div>
										</div>
									</td>
									<td className="px-5 py-3.5 text-[13px] text-[#475569]">{s.category}</td>
									<td className="px-5 py-3.5 text-right text-[13px] font-semibold text-[#0F172A]">Rp {(s.totalTransaction / 1000000).toFixed(1)}jt</td>
									<td className="px-5 py-3.5">
										<span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full ${status.color}`}>
											<span className={`w-1.5 h-1.5 rounded-full ${s.status === "active" ? "bg-[#22C55E]" : s.status === "pending" ? "bg-[#F59E0B]" : "bg-[#94A3B8]"}`}></span>
											{status.label}
										</span>
									</td>
								</tr>
